...
namespace JiraApiOpenSourseLibrary.JiraRestClient
{
    ...
    public class JiraClient<TIssueFields> : IJiraClient<TIssueFields> where TIssueFields : IssueFields, new()
    {
        ...
        public IEnumerable<Issue<TIssueFields>> EnumerateIssuesByQuery(String jqlQuery, String[] fields, Int64 startIndex)
        {
            try
            {
        ...
        private IEnumerable<Issue<TIssueFields>> EnumerateIssuesByQueryInternal(String jqlQuery, String[] fields, Int64 startIndex)
        {
            var queryCount = 50;
        ...
        public Issue<TIssueFields> UpdateIssue(Issue<TIssueFields> issue)
        {
            try
            {
                var path = String.Format("issue/{0}", issue.key);
        ...
        public List<History> GetChangelog(IssueRef issue)
        {
            try
            {
                var path = String.Format("issue/" + issue.key +"?expand=changelog");
                var request = CreateRequest(Method.GET, path);

                var response = ExecuteRequest(request);
                AssertStatus(response, HttpStatusCode.OK);

                return deserializer.Deserialize<RootObject>(response).changelog.histories;
            }
            catch (Exception ex)
            {
                Trace.TraceError("GetWatchers(issue) error: {0}", ex);
                throw new JiraClientException("Could not load histories", ex);
            }
        }
        ...
        public Comment CreateComment(Issue issue, String comment, Visibility objVisibility)
        {
            try
            {
                var path = String.Format("issue/{0}/comment", issue.id);
                var request = CreateRequest(Method.POST, path);
                request.AddHeader("ContentType", "application/json");
                request.AddBody(new Comment { body = comment, visibility = objVisibility });
